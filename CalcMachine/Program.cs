      ...
      Console.WriteLine(@"
       ...
       switch (SpecialAlgebra)    
       {
        ...
        if (Δ >= 0)
        {
         ...
         double LogarithmX = Convert.ToDouble(Console.ReadLine());   
         double LogBase = Convert.ToDouble(Console.ReadLine());
          Console.WriteLine($"The Logarithm of {LogarithmX} with a {LogBase} base is {Math.Log(LogarithmX, LogBase)}");
         break;
         case "3":
